...
class DocumentSet(BaseModel):
    ...
    submission_date: datetime = datetime.now()
    last_update: datetime = datetime.now()
    share_state = enumerate(["private", "public", "embargoed", "restricted"])   #!!!!!
    owner: str
...
documents_doi_sets = Dict[str, str] # dictionary of DOIs of the documents and the DOI of the set they belong to (key is the set DOI)
...
def is_valid_user(user_id: uuid.UUID, token: str = Depends(oauth2_scheme)):
...
@app.post("/documents/")
async def create_document(file: UploadFile = File(...), user_id: uuid.UUID = None, date_time: datetime = datetime.now(), share_state: str = "private", access_control_list = None, token: str = Depends(oauth2_scheme)):
    ...
    document = Document(name = file.filename, content = content, doi = doi, type = file.content_type, size = file.size, submission_date = datetime.now(), last_update = datetime.now(), access_control_list = access_control_list, history = ["created by " + users[user_id].username + " on " + str(datetime.now())])
    if set_name in documents_set.keys():
        ...
        document_set = DocumentSet(doi = set_doi, set_name = set_name, collection = [document], num = 1, submission_date = datetime.now(), last_update = datetime.now(), share_state = share_state, owner = user_name, access_control_list = None, access_from_data = None)
...
async def get_document(doi: str, user_id: uuid.UUID, token: str = Depends(oauth2_scheme)):
    ...
    if doi in sets_doi.keys(): #it is a set
        ...
        if doc_set.access_control_list is None or (users["role"] == "admin" or users["username"] in doc_set.access_control_list) or doc_set.owner == users[user_id].username:
        ...
        if document.access_control_list is None or (users["role"] == "admin" or users["username"] in document.access_control_list) or doc_set.owner == users[user_id].username:
            access_logs.append({"doi": doi, "access_date": datetime.now(), "user": users[user_id].username})
...
async def filter_documents(type: Optional[str] = None, size: Optional[int] = None, owner: Optional[str] = None, user_id: uuid.UUID = None, token: str = Depends(oauth2_scheme)):
    if not is_valid_user(user_id, token):
        raise HTTPException(status_code = 404, detail = "Invalid user")
    result = []
    for doc in documents_doi_sets.values():
